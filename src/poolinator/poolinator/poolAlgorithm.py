...
class PoolAlgorithm:
    ...
    def check_obstacle(self, ball_1, ball_2, dist, ball_ignore=None):
        ...
        for _, value in self.balls.items():
            ...
            if not (min(ball_1.x, ball_2.x) <= value.x <= max(ball_1.x, ball_2.x)):
                continue
    ...
    def calc_cue_pos(self, target_ball, impact):
        ...
        if self.check_obstacle(self.cue_ball_pos, cf, self.D_ball, target_ball):
            return possible, neutral_cue, strike_ang

        # Calculate striking angle
        strike_ang = calc_ang(self.cue_ball_pos, cf)
        sharpness = abs(pocket_ang - strike_ang)  # check that angle of shot is reasonable
    ...
    def calc_cue_targ(self):
        ...
        for key1, value1 in self.balls.items():
            if (key1 == "cue_ball"):
                continue
            for i in range(N):
                possible, cue, strike_ang = self.calc_cue_pos(value1, self.pockets[i])
                if possible:
                    return cue, strike_ang, key1

    def test_strike_pose(self, impact):
        """
